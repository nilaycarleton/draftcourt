...
Status: Phase 3F (covers Phase 0–3E2: foundation, data model, leagues,
draft core + outbox, preferences, CPU mocks, guest demos, history/analysis,
replay timeline, private share capabilities). Deltas after Phase 2 are
appended below; the per-phase migration list is authoritative — source of
truth is always `packages/db/prisma/schema.prisma`.
