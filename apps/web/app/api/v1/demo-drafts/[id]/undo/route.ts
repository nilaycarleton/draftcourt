...
/**
 * Undo the latest pick in a guest demo draft (Phase 3D).
 */
...
function errorToProblem(error: unknown): NextResponse | null {
  ...
  // Version conflict is the most specific check — must precede generic message checks.
  if (
  ...
  if (error instanceof Error) {
    if (
      ...
      error.message.includes("no pick to undo") ||
      error.message.includes("not the latest") ||
...
export async function POST(
  ...
  try {
    const result = await undoDemoPick({
      draftId: id,
      capabilityToken,
    ...
    await writeAuditLog({
      actorId: null,
      action: "demo.draft.undo",
      entityType: "demo_draft",
      entityId: id,
      before: null,
      after: { sequence: result.authoritative.currentSequence },
      traceId,
    });
    ...
    console.error("[demo-drafts] undo failed", error);
