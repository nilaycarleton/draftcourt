...
/**
 * Complete a guest demo draft (Phase 3D).
 *
 * Requires valid capability token, If-Match, Idempotency-Key.
 * Board must be full (all picks made).
 */
...
export async function POST(
  ...
  try {
    const result = await completeDemoDraft({
      draftId: id,
      capabilityToken,
    ...
    console.error("[demo-drafts] complete failed", error);
